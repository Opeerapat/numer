...
class simpson extends Component {
    constructor(props) {
        super(props);
        this.state = { sd: '', count: 0, value: '', fxl: [], fxr: [], xr: [], xl: [], fx: '', n: '2', movie: '', err: [], showtable: false, showgra: false, showfx: false };
    ...
    onChangeadd = async () =>
  {
    ...
    onChangeSub() {
        this.state.showfx = true;
        var k = 0, errord = 0, che = 0, chf = 0, p = 0, sum = 0;
        ...
        for (p = 0; p <= nx; p++) {
            ...
            if (xl == xlo || xl == xro) {
                ...
                sum = sum + (4 * this.functionfx({ x: xl }));
            }
            console.log("sum:" + sum);
            xl = xl + id;
