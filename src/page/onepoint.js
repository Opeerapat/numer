...
class onepoint extends Component {
  constructor(props) {
    super(props);
    this.state = {sd: '', count: 0, value: '', fxl: [], xl: [], fx: '', movie: '', err: [], showtable: false, showgra: false,showfx:false,showfx:false };
  ...
  componentDidMount = async () => {
    await api.getMovieById("5e68f802cc007921083c50c8").then(db => {
      this.setState({
        fx: db.data.data.fx
      })
  ...
  onChangeSub() {
    ...
    chf++;
    do {
      this.state.xl[i] =this.functionfx({x:xl});
